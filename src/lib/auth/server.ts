...
/** Native Google OAuth (production) — callback stays on examhub.shop */
...
/** Strip trailing slash so origin comparisons match the browser. */
function normalizeOrigin(url: string): string {
  return url.trim().replace(/\/+$/, "");
}

/**
 * Expand a public site URL into trusted variants:
 * - exact origin
 * - with/without `www.`
 * Used so custom domains (examhub.shop + www.examhub.shop) both work.
 */
function originVariants(url: string): string[] {
  const base = normalizeOrigin(url);
  ...
  try {
    ...
    if (host.startsWith("www.")) {
      ...
      out.add(normalizeOrigin(bare.toString()));
    } else if (host.includes(".")) {
      const www = new URL(base);
      www.hostname = `www.${host}`;
      out.add(normalizeOrigin(www.toString()));
    }
  } catch {
    /* ignore bad URL */
  }
...
const explicitBaseURL = explicitBaseURLRaw
  ? normalizeOrigin(explicitBaseURLRaw)
  : undefined;
// Render injects this for the default *.onrender.com hostname
const renderExternalUrl = env("RENDER_EXTERNAL_URL")
  ? normalizeOrigin(env("RENDER_EXTERNAL_URL")!)
  : undefined;
// Extra origins: comma-separated full origins
...
const trustedOrigins: string[] = explicitBaseURL
  ? [
      ...
      ...(renderExternalUrl ? originVariants(renderExternalUrl) : []),
      ...extraTrusted.flatMap((o) => originVariants(o)),
      // Common custom domain for this project (safe extras; no secrets)
      "https://www.examhub.shop",
      "https://examhub.shop",
      ...LOCAL_DEV_ORIGINS,
    ]
  : [
...
// Grok broker OAuth is for live-preview sandbox only.
// Production (examhub.shop) uses native Google when GOOGLE_CLIENT_ID/SECRET are set —
// so users return to examhub.shop, never grok.com.
const useGrokBroker = authConfigured && !nativeGoogleEnabled;
...
export const auth = betterAuth({
  ...
  // Native Google → callback is {BETTER_AUTH_URL}/api/auth/callback/google
  // e.g. https://www.examhub.shop/api/auth/callback/google
  ...(nativeGoogleEnabled
    ? {
        socialProviders: {
          google: {
...
// Seed admin email/password after auth + DB are available
if (typeof window === "undefined") {
  void import("./ensure-admin").then((m) => m.ensureAdminAccount()).catch(() => {});
}
